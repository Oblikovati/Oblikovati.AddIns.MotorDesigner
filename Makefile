# Build the oblikovati-motor-designer add-in as a c-shared library (.so/.dll/.dylib) and
...
# version is independent of the host (pinned to match the head's 1.27, see go.mod).
...
LIB := oblikovati-motor-designer.$(EXT)
OUT ?= build
